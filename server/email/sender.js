...
self.sendSubscriptionReceipt = function (donationId) {
  var d = $q.defer();
    ...
    (newDonation, next) => {
      if (newDonation.IsApproved) {
        ...
        var data = {
          ...
          template: 'user-subscription-receipt-email',
          subject: `Recibo de Contribuição - ${mailchimp.nameCase(moment(newDonation.DonationDate).format('MMM/YYYY'))}`,
          context: {
            name: user.name.split(' ')[0],
            fullName: user.FullName,
        ...
        transporter.sendMail(data, function (err) {
          if (!err) {
            console.log('Email de assinatura aprovada enviado para', user.email);
            Donation.update({
              SentEmail: 1
            }, {
              where: {
